...
void push_condition_one(t_stacks **stack_a, t_stacks **stack_b, t_stacks *cheap_node, int condition_num)
{
    if(condition_num == 1)
    {
        while((node_pos(*stack_a, cheap_node->target_node) != 0) && (node_pos(*stack_b, cheap_node->num) != 0))
...
void push_to_a(t_stacks **stack_a, t_stacks **stack_b, t_stacks *cheap_node)
{
    ...
    if(index_a > 0 && index_b > 0 && index_a <= median_a && index_b <= median_b)
        push_condition_one(stack_a, stack_b, cheap_node, 1);
    else if(index_a > 0 && index_b > 0 && index_a > median_a && index_b > median_b)
        push_condition_one(stack_a, stack_b, cheap_node, 2);
    if(cheap_node->main_index <= median_b)
        push_condition_two(stack_a, stack_b, cheap_node, 1);
    else 
        push_condition_two(stack_a, stack_b, cheap_node, 2);
    if(cheap_node->target_index <= median_a)
        push_condition_three(stack_a, stack_b, cheap_node, 1);
    else 
        push_condition_three(stack_a, stack_b, cheap_node, 2);
    pa_op(stack_a, stack_b);
}

void final_sort(t_stacks **stack_a)
{
    int min = return_min_value(*stack_a);
    int min_index = node_pos(*stack_a, min);
    int median = stack_size(*stack_a) / 2;
    if(min_index <= median)
    {
        while(min_index-- > 0)
            ra_op(stack_a, 0);
